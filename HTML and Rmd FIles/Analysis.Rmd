...
allEnrichGO <- function(data){
  ...
  for(i in 1:length(goTypes)){
    if(i == 1){
       goDF = enrichGO(gene = gene.df$ENTREZID, universe = universe$ENTREZID, ont = goTypes[i], OrgDb = org.Mm.eg.db)
       goDF = goDF %>% as.tibble() %>% mutate('Category' = goTypes[i])
    } else{
      temp = enrichGO(gene = gene.df$ENTREZID, universe = universe$ENTREZID, ont = goTypes[i], OrgDb = org.Mm.eg.db)
      temp = temp %>% as.tibble() %>% mutate('Category' = goTypes[i])
      goDF = rbind(goDF, temp)
    }
  }
  goDF = goDF %>% rowwise() %>% dplyr::mutate(zscore = zScore(geneID, data))
}
...
ggplot(test, aes(x = zscore, y = -log10(p.adjust), size = Count, colour = Category)) + geom_point(alpha = 0.15) + theme_bw() + facet_wrap(~Category)
